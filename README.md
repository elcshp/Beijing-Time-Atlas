...
The preparation scripts require Python, NumPy, SciPy, Node.js and the original OSM downloads. In this download, the scripts read the included OpenStreetMap snapshots from the project-relative `raw/` directory. The delivered static data can be used without running those scripts. Re-running the base generator directly over `data.json` can replace enriched geometry; generate to a separate output and deliberately merge the intended fields.
