...
function changeLanguage(value){if(!['en','zh'].includes(value))throw Error('Unsupported language');language=value;try{localStorage.setItem('beijing-map-language',language)}catch{}applyLanguage();$('mobile-route-toggle').setAttribute('aria-label',tr(document.querySelector('.route-search').classList.contains('is-open')?'routeCollapse':'routeExpand'));fillPlaceSelectors();sync();refreshStats();updatePair();if(activeLandmark!==null&&!$('landmark-card').hidden)showLandmark(activeLandmark);hideTooltip();routeUI?.refresh();render();}
const compactLayout=window.matchMedia('(max-width: 900px)');
const phoneLayout=window.matchMedia('(max-width: 700px)');
const dayControls=document.querySelector('.day-controls'),dayHome=dayControls.parentNode,dayNext=dayControls.nextSibling;
const options=$('show-field').closest('.map-options'),optionsHome=options.parentNode,optionsNext=options.nextSibling;
const insights=document.querySelector('.insights'),insightsHome=insights.parentNode,insightsNext=insights.nextSibling;
const mobileRoute=$('mobile-route-toggle'),routeSearch=document.querySelector('.route-search');
function setMobileRoute(open){routeSearch.classList.toggle('is-open',open);mobileRoute.setAttribute('aria-expanded',String(open));mobileRoute.setAttribute('data-i18n-aria',open?'routeCollapse':'routeExpand');mobileRoute.setAttribute('aria-label',tr(open?'routeCollapse':'routeExpand'));mobileRoute.textContent=open?'×':'⌕';}
mobileRoute.onclick=()=>setMobileRoute(!routeSearch.classList.contains('is-open'));
$('route-pair').addEventListener('click',()=>{if(compactLayout.matches)setMobileRoute(true)});
function applyResponsiveLayout(){const compact=compactLayout.matches,phone=phoneLayout.matches;$('controls-panel').open=!compact;
 if(phone){$('mobile-day').appendChild(dayControls);document.querySelector('.controls-body').append(options,$('pair-panel'),insights);}
 else{dayHome.insertBefore(dayControls,dayNext);optionsHome.insertBefore(options,optionsNext);insightsHome.insertBefore(insights,insightsNext);$(compact?'mobile-pair':'desktop-pair').appendChild($('pair-panel'));setMobileRoute(false);}
 $(compact?'mobile-details':'stage').appendChild($('landmark-card'));hideTooltip();requestRender();}
compactLayout.addEventListener('change',applyResponsiveLayout);phoneLayout.addEventListener('change',applyResponsiveLayout);applyResponsiveLayout();
